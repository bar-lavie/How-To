...
const HowToBox = (props) => (
  <Link
    ...
    className="my-2 p-4 flex items-center bg-white no-underline text-indigo-700 transition duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-md"
  >
    <span className="p-4 mr-4 bg-gray-300 capitalize font-bold relative">
      <span className="xy-align">{props.data.name.charAt(0)}</span>
    </span>
    <span>
      <span className="capitalize text-md font-bold block leading-tight">
        {props.data.name}
      </span>
      <span className="text-xs block mt-2">{props.data.steps.length} Steps</span>
    </span>
